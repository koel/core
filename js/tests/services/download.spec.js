...
describe('services/download', () => {
  ...
  it('downloads songs', () => {
    ...
    download.fromSongs([factory('song', { id: 'foo' }), factory('song', { id: 'bar' })])
  ...
  it('downloads all by artist', () => {
    const triggerMock = mock(download, 'trigger')
    download.fromArtist(factory('artist', { id: 42 }))
  ...
  it('downloads all in album', () => {
    const triggerMock = mock(download, 'trigger')
    download.fromAlbum(factory('album', { id: 42 }))
  ...
  each([[[], false], [factory('song', 5), true]]).test('downloads playlist if available', (songs, triggered) => {
    const triggerMock = mock(download, 'trigger')
    download.fromPlaylist(factory('playlist', { id: 42, songs }))
