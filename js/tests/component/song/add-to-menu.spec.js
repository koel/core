...
describe('components/song/add-to-menu', () => {
  ...
  it('queue songs after current', () => {
    ...
    const closeStub = stub(wrapper.vm, 'close')
  ...
  it('add songs to existing playlist', () => {
    ...
    const wrapper = initComponent()
    const addSongsStub = stub(playlistStore, 'addSongs')
    const closeStub = stub(wrapper.vm, 'close')
  ...
  it('creates new playlist from songs', async done => {
    const storeStub = stub(playlistStore, 'store').callsFake(() => {
      return new Promise(resolve => resolve(factory('playlist')))
    })
    const wrapper = initComponent()
    const closeStub = stub(wrapper.vm, 'close')
